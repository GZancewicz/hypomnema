...
def extract_homily_number(div_elem):
    ...
    # Check the id attribute for patterns like "iii.iv" (Homily IV)
    div_id = div_elem.get('id', '')
    if div_id and 'iii.' in div_id:
        # Special case for iii.iv which is Homily 1
        if div_id == 'iii.iv':
            return 1
        # Pattern: iii.v = Homily 2, iii.vi = Homily 3, etc.
        parts = div_id.split('.')
        if len(parts) >= 2:
            roman = parts[1].upper()
            ...
            if arabic:
                # Adjust for the offset (iii.v is Homily II)
                if arabic >= 5:  # v=5 is actually Homily 2
                    return arabic - 3
                return arabic
            ...
            if homily_num:
                ...
                for note in div.iter('note'):
                    ...
                    if note_n:  # Only process notes with a number
                        # Extract content from the note, excluding scripRef tags
...
def main():
    xml_file = '../texts/commentaries/chrysostom/matthew/chrysostom_matthew_homilies.xml'
    output_file = '../texts/commentaries/chrysostom/matthew/all_footnotes.json'
