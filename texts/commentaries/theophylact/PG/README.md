...
- **Skills & tooling** (`.claude/skills/`, now git-tracked):
  ...
  - `theophylact-lemma-verses/` — verse-coverage derivation. `match_verses.py` (deterministic Greek→**Patriarchal** cosine, no LLM) + `overrides_matthew.json` (adjudicated corrections) → `coverage.json`.
...
- ✅ **Verse coverage — Matthew done** (1124 lemmata → `coverage.json`, Greek→Patriarchal match + 15 adjudications). Mark/Luke/John pending.
...
- **Run verse coverage** for Mark, Luke, John: `match_verses.py --all --flags` → adjudicate weak/none into `overrides_<gospel>.json` → `--coverage-json` → each gospel's `coverage.json`.
