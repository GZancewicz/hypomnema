#!/usr/bin/env python3
"""
Parse scripture references from Cyril Luke sermons.
Generates scripture_references.json files for each sermon.
"""
...
def process_sermons():
    """Process all sermons and extract scripture references."""
    ...
    for sermon_num in range(1, 154):  # Luke has 153 sermons
        ...
        if not content_file.exists():
            print(f"Sermon {sermon_num}: No content.json found")
            continue
        ...
        sermons_processed += 1
        print(f"Sermon {sermon_num:3d}: {len(all_refs)} references found")
    
    print(f"\nTotal sermons processed: {sermons_processed}")
    print(f"Total references extracted: {total_references}")

def main():
    print("Extracting scripture references from Cyril Luke...")
    print("-" * 50)
    process_sermons()
