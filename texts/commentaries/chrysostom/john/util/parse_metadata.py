#!/usr/bin/env python3
"""
Parse Chrysostom John homilies and extract metadata.
    ...
    if subtitle is not None:
        ...
        if match:
            ...
            return {
                ...
                'display': f"John {start_chapter}:{start_verse}" + 
...
def get_default_reference(homily_num):
    ...
    if homily_num in coverage_map:
        ...
        return {
            ...
            'start': ref['start'],
            'end': ref['end'],
            'display': f"John {ref['start']['chapter']}:{ref['start']['verse']}-{ref['end']['chapter']}:{ref['end']['verse']}"
        }
...
def main():
    print("Parsing Chrysostom John metadata...")
    print("-" * 50)
    process_xml_source()
