#!/usr/bin/env python3
"""Extract scripture references from Chrysostom's John homilies XML"""

import json
import re
from pathlib import Path

def parse_homily_id_john(href):
    """Extract homily number from href like '#iv.xxviii-p7.2' or '#v.vi-p41.1'"""
    # Pattern: iv.{roman} or v.{roman}
    match = re.search(r'(?:iv|v)\.([ivxlcdm]+)', href, re.IGNORECASE)
    if match:
...
def main():
    print("Extracting scripture references from Chrysostom's John homilies...")

    base_dir = Path(__file__).parent.parent
    xml_file = base_dir / 'source' / 'chrysostom_john_homilies.xml'
    ...
    for line in lines:
        ...
        if current_book:
            for ref_match in re.finditer(ref_pattern, line):
                ...
                homily_num = parse_homily_id_john(href_id)
                if homily_num:
    ...
    homilies = set(ref['homily'] for ref in references)
    print(f"\nStatistics:")
    print(f"  Total references: {len(references)}")
    print(f"  Unique books: {len(books)}")
    print(f"  Homilies referenced: {min(homilies)}-{max(homilies)}")
